...
Cart.prototype.create = () => this.stripe.orders.create({
  currency: 'eur',
  items: [
    {
      type: 'sku',
...
Cart.prototype.retreive = () => this.stripe.orders.retreive(this.orderId, (err, order) => {
  if (err) {
    console.log(err);
    return;
  }
...
Cart.prototype.add = () => this.stripe.orders.update(this.orderId, {
  items: [
    {
      type: 'sku',
      parent: 'sku_1',
...
Cart.prototype.subtract = () => this.stripe.orders.update(this.orderId, {
  items: [
    {
      type: 'sku',
      parent: 'sku_1',
